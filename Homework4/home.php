...
if($has_session){
	...
	// Get status
	if($stmt = $mysqli->prepare("SELECT * from statuses where user_id=?")){
		if($stmt->bind_param("i", $_SESSION['user_id'])){
			if(!$stmt->execute()){
				die("Error - Issue executing prepared statement: " . mysqli_error($mysqli));
			}
...
<?php echo '<div id="id" class="'. $id_to_get .'">' . $token. '</div>' ?>
	<div id="bluebar">
		...
		<div id="logout_div">
			<a id="logout" href="/armbook/logout.php">Logout</a>
		</div>
	</div>
	<div id="global">
			<div id="basic_profile">
				...
				if($_SESSION['user_id'] == $id_to_get){
					...
					echo '<div id="change_profile" class="change_prof">
						<h3>Change your Profile Picture</h3>
						<div id="update"> </div>
						<form id="update_prof">
							url to image: <input id="url" class="prof_image" placeholder="http://www.example.com/image.png" type="text" />
							<br>
							<input class="float_input" type="submit" />
						</form>
					</div>
					';
				}
